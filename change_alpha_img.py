...
def change_img(image, count):
    save_name = save_path + 're' + image[:image.find('_')] + '_%04d.png'%count
    ...
    for i in range(0, h):
            for j in range(0, w):
                alpha_array[i][j] = ((float(img[i][j][3]))/max_org)**0.4
...
long = len(os.listdir(image_path))
for i in os.listdir(image_path):
    change_img(i, n)
    print i + ' was reshaped (%d/%d)'%(n+1, long)
    n += 1
